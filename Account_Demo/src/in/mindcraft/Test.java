	...
	public static void main(String[] args) {
		...
		while(true) 
		{
			...
			case 4:
			    ...
			    for (int i = 0; i < count; i++) {
			   if (acc[i].getAccountHolderNo() == acc_no) {     
					 if (acc[i].getAccountBalance()>=amount) {
			                acc[i].withdraw(amount);
				       System.out.println("Withdrawal successful. New balance:account: " +acc[i].getAccountBalance());
					 }
			   }
					 else {
